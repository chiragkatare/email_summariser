...
def upgrade():
    ...
    op.add_column('accountant', sa.Column('new_id', postgresql.UUID(as_uuid=True), default=sa.text('uuid_generate_v4()')))
    op.add_column('item', sa.Column('new_id', postgresql.UUID(as_uuid=True), default=sa.text('uuid_generate_v4()')))
    op.add_column('item', sa.Column('new_owner_id', postgresql.UUID(as_uuid=True), nullable=True))

    # Populate the new columns with UUIDs
    op.execute('UPDATE "accountant" SET new_id = uuid_generate_v4()')
    op.execute('UPDATE item SET new_id = uuid_generate_v4()')
    op.execute('UPDATE item SET new_owner_id = (SELECT new_id FROM "accountant" WHERE "accountant".id = item.owner_id)')

    # Set the new_id as not nullable
    op.alter_column('accountant', 'new_id', nullable=False)
    ...
    op.drop_column('accountant', 'id')
    op.alter_column('accountant', 'new_id', new_column_name='id')

    op.drop_column('item', 'id')
    op.alter_column('item', 'new_id', new_column_name='id')

    # Create primary key constraint
    op.create_primary_key('accountant_pkey', 'accountant', ['id'])
...
def downgrade():
    ...
    op.add_column('accountant', sa.Column('old_id', sa.Integer, autoincrement=True))
    ...
    op.execute('CREATE SEQUENCE IF NOT EXISTS accountant_id_seq AS INTEGER OWNED BY "accountant".old_id')
    op.execute('CREATE SEQUENCE IF NOT EXISTS item_id_seq AS INTEGER OWNED BY item.old_id')

    op.execute('SELECT setval(\'accountant_id_seq\', COALESCE((SELECT MAX(old_id) + 1 FROM "accountant"), 1), false)')
    op.execute('SELECT setval(\'item_id_seq\', COALESCE((SELECT MAX(old_id) + 1 FROM item), 1), false)')

    op.execute('UPDATE "accountant" SET old_id = nextval(\'accountant_id_seq\')')
    op.execute('UPDATE item SET old_id = nextval(\'item_id_seq\'), old_owner_id = (SELECT old_id FROM "accountant" WHERE "accountant".id = item.owner_id)')
    ...
    op.drop_column('accountant', 'id')
    op.alter_column('accountant', 'old_id', new_column_name='id')
